...
class Scheduler:
    ...
    def _schedule_default(self) -> SchedulerOutputs:
        ...
        if len(prefills.seq_groups) == 0:
            ...
            # fairness
            self.update_counter(running_scheduled)
    ...
    # fairness
    def update_counter(self, decode_state_seqs: SchedulerRunningOutputs) -> None:
        pass
...
class VTCScheduler(Scheduler):
    ...
    def add_seq_group(self, seq_group: SequenceGroup) -> None:
        # Add sequence groups to the waiting queue.
        self.waiting.append(seq_group)
        if seq_group.client_id not in self.user_req_list:
            self.user_req_list[seq_group.client_id] = deque([seq_group])
            self.served[seq_group.client_id] = 0
        else:
            self.user_req_list[seq_group.client_id].append(seq_group)

        # waiting queue was empty before
        if len(self.user_req_list[seq_group.client_id]) == 1:
            # lift counter
            cnts = [v for k, v in self.served.items()
                      if (len(self.user_req_list[k]) > 0 and k != seq_group.client_id)]
            if len(cnts) > 0:
    ...
    # fairness
    def update_counter(self, decode_state_seqs: SchedulerRunningOutputs) -> None:
        for seq_qroup in decode_state_seqs.decode_seq_groups_list:
                self.served[seq_qroup.client_id] += 1 * self.output_price